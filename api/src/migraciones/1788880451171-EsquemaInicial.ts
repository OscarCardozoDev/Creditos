...
export class EsquemaInicial1788880451171 implements MigrationInterface {
  name = 'EsquemaInicial1788880451171';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`CREATE INDEX "IX_Sesiones_Expiradas" ON "Sesiones" ("expira_en") `);
    await queryRunner.query(
    ...
    await queryRunner.query(`CREATE INDEX "IX_Creditos_Deudor" ON "Creditos" ("deudor_id") WHERE eliminado_en IS NULL`);
    await queryRunner.query(
    ...
    await queryRunner.query(`CREATE INDEX "IX_Historial_Credito" ON "HistorialCredito" ("credito_id", "fecha" DESC) `);
    await queryRunner.query(`CREATE TABLE "Cuotas" ("cuota_id" bigint NOT NULL IDENTITY(1,1), "credito_id" uniqueidentifier NOT NULL, "numero_cuota" int NOT NULL, "fecha_vencimiento" date NOT NULL, "valor_cuota" decimal(18,2) NOT NULL, "abono_capital" decimal(18,2) NOT NULL, "abono_interes" decimal(18,2) NOT NULL, "saldo_posterior" decimal(18,2) NOT NULL, "estado" varchar(12) NOT NULL CONSTRAINT "DF_ffd3ef1ae5fad752063d23b4d56" DEFAULT 'PENDIENTE', "fecha_pago" date, "valor_pagado" decimal(18,2), CONSTRAINT "UQ_Cuotas_Numero" UNIQUE ("credito_id", "numero_cuota"), CONSTRAINT "CK_Cuotas_Pago" CHECK ((estado = 'PAGADA' AND fecha_pago IS NOT NULL AND valor_pagado IS NOT NULL)
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`ALTER TABLE "Credenciales" DROP CONSTRAINT "FK_Credenciales_Usuario"`);
    await queryRunner.query(`ALTER TABLE "Cuotas" DROP CONSTRAINT "FK_Cuotas_Credito"`);
    await queryRunner.query(`ALTER TABLE "HistorialCredito" DROP CONSTRAINT "FK_Historial_Usuario"`);
    await queryRunner.query(`ALTER TABLE "HistorialCredito" DROP CONSTRAINT "FK_Historial_Credito"`);
    await queryRunner.query(`ALTER TABLE "Notificaciones" DROP CONSTRAINT "FK_Notif_Credito"`);
    await queryRunner.query(`ALTER TABLE "Creditos" DROP CONSTRAINT "FK_Creditos_Deudor"`);
    ...
    await queryRunner.query(`DROP INDEX "IX_Cuotas_Pendientes" ON "Cuotas"`);
    await queryRunner.query(`DROP TABLE "Cuotas"`);
    await queryRunner.query(`DROP INDEX "IX_Historial_Credito" ON "HistorialCredito"`);
    await queryRunner.query(`DROP TABLE "HistorialCredito"`);
    await queryRunner.query(`DROP INDEX "IX_Notif_Pendientes" ON "Notificaciones"`);
    await queryRunner.query(`DROP TABLE "Notificaciones"`);
    await queryRunner.query(`DROP INDEX "IX_Creditos_Estado_Fecha" ON "Creditos"`);
    await queryRunner.query(`DROP INDEX "IX_Creditos_Deudor" ON "Creditos"`);
    await queryRunner.query(`DROP INDEX "UX_Creditos_Duplicado" ON "Creditos"`);
    await queryRunner.query(`DROP TABLE "Creditos"`);
    await queryRunner.query(`DROP INDEX "IX_Sesiones_Usuario" ON "Sesiones"`);
    await queryRunner.query(`DROP INDEX "IX_Sesiones_Expiradas" ON "Sesiones"`);
